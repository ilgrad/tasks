        ...
        navbarPage("EFAshiny",inverse = F, fluid = T,
                   tabPanel("Introduction",
                            ...
                            mainPanel(tabsetPanel(id  ="",
                                                  tabPanel("Data format", value = 1,
                                                           HTML('<h3>Load Data</h3>
      <p>Click Browse to load a data file. </p>
      <p></p>
      <p>Data may be respondent level data or a correlation grid.  Respondent level data may be loaded from a
         SPSS .sav or a .csv file:</p>'),
                                                           img(src = "SPSS_Dataset.png"),
                                                           br(),
                                                           HTML('<p>Example SPSS (.sav) file</p>'),
                                                           br(),
                                                           img(src = "CSV_Dataset.png"),
                                                           HTML('<p>Example comma delimited (.csv) file</p>'),
                                                           br(),
                                                           HTML('<p>A correlation matrix may be loaded from a .csv file.  The first row and column should contain variable
               names and/or labels.  The correlation grid must be square (number of rows = number of columns).</p>'),
                                                           img(src = "Corr_Matrix.png"),
                                                           HTML('<p>Example comma delimited (.csv) file with correlation matrix</p>')
                                                  ),
                   ...
                   tabPanel("Data Input",
                        ...
                        fileInput("file", "Upload data-file (csv, xlsx or sav):",
                        accept = c("text/csv",
                        "text/comma-separated-values,text/plain",
                        'application/xlsx', ".xlsx",".csv", ".sav", ".txt")),
                        h4("CSV settings only", align = "center"),
                        fluidRow(
                                column(width = 6,
                                       radioButtons('sep', '1. Separator:',
                                                    c('comma( , )'=',',
                                                      'Semicolon ( ; )'=';',
                                                      'Tab( \\t )'='\t'),
                                                    selected = ',')),
                                column(width = 6,
                                       radioButtons('dec', '2. Decimal separator:',
                                                    c('comma( , )'=",",
                                                      'point( . )'="."), selected='.'))
                        ),
                        radioButtons('hdr', "3. Header of variable",
                                     c("TRUE","FALSE"),"TRUE"),
                        br(),
                        br(),
                        # correlation matrix
                        radioButtons('datatype', 'Type of any Data file',
                                     c("Raw Data",
                                       "Correlation Matrix"),
                                     "Raw Data"),
                        # variable selection
                        ...
                        br(),
                        a(href="dmda.example.xlsx", "Download example", download=NA, target="_blank")
                        ),
