shinyServer(function(input, output) {
        #### csv settings ####
        csv_settings <- reactiveValues()
        observeEvent(input$sep, {
                csv_settings$sep <- input$sep
                csv_settings$dec <- input$dec
        })
        
        Dataset <- reactive({
                if (!is.null(input$file)) {
                        type_file <- tail(input$file$name %>% strsplit(".", fixed = TRUE)  %>%  unlist, n=1)
                        ...
                        if (type_file == "csv"){
                                if (input$hdr == "TRUE"){ 
                                Dataset <- read.csv(input$file$datapath, header = T, stringsAsFactors = F, sep=csv_settings$sep,  dec=csv_settings$dec)
                                }
                                if (input$hdr == "FALSE"){ 
                                        Dataset <- read.csv(input$file$datapath, header = F, stringsAsFactors = F, sep=csv_settings$sep,  dec=csv_settings$dec)
                                }
                        } else if (type_file == "xlsx") {
                                Dataset <- read_xlsx(input$file$datapath, sheet = NULL, range = NULL, col_names = TRUE)
                        }  else if (type_file %in% c("sav", "SAV")) {
                                Dataset <- as.data.frame(haven::read_spss(input$file$datapath))
                        }
                        ...
                        return(read_xlsx("www/dmda.example.xlsx", sheet = NULL, range = NULL, col_names = TRUE))
                }
        })
        ...
        output$varselect <- renderUI({
                ...
                selectizeInput("vars", "Variables to include:", choices=names(Dataset()),selected = tail(names(Dataset()),-1), multiple = TRUE,
                               options = list(
        ...
        observe(output$itemdist <- renderPlotly({
                ...
                if (!all(sapply(D(),class) %in% c("numeric","integer"))) { stop("All input variables should be numeric or integer")}
                        ...
                        dist1 <- ggplot(dtalong, aes(x = Response, fill = Item))+
                                ...
                                theme_default()+
                                labs(list(y = " ",x = " "))
                        ggplotly(dist1, height = input$ploth1,width = input$plotw1)
        }))
        ...
        observe(output$itemdensity <- renderPlotly({
                ...
                dist2 <- ggplot(dtalong, aes(x = Response, fill = Item))+
                        ...
                        theme_default()+
                        labs(list(y = " ",x = " "))
                ggplotly(dist2, height = input$ploth1,width = input$plotw1)
        }))
